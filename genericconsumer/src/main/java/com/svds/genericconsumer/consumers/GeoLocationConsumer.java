...
public class GeoLocationConsumer extends Consumer {
    ...
    public void init(KafkaStream stream, int threadNumber, String[] parameters) {
        ...
        try {
            Configuration conf = createHBaseConfig(zooKeeperQuorum);
            HBaseAdmin hba = new HBaseAdmin(conf);
            if (hba.tableExists(hbaseTable) == false) {
                HTableDescriptor tableDescriptor = new HTableDescriptor(hbaseTable);
                HColumnDescriptor columnDescriptor = new HColumnDescriptor("locations".getBytes());
                tableDescriptor.addFamily(columnDescriptor);
                hba.createTable(tableDescriptor);
            }
            setTable(new HTable(conf, hbaseTable));
        } catch (Exception e) {
